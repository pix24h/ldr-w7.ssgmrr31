...
if(jurisdictionName !== undefined && jurisdictionName !== null && jurisdictionName !== '') jurisdictionName.toLowerCase();
$('body').addClass(`jurisdiction-${jurisdictionName}`);

let globalParams = JSON.parse(document.getElementById('GlobalParameters').value);

let portalName = $('body').data('portalname');
let captionFreeBetText4 = GetCaption('jetx.board.freebet.text4');
let captionFreeBetUse = GetCaption('jetx.board.freebet.use');
let captionFreeBetText4Continue = GetCaption('jetx.board.freebet.text4.continue');
let captionFreeBetContinue = GetCaption('jetx.board.freebet.continue');

if(portalName === 'betlive') {
    $('.caption-freebet').html(GetCaption(`${portalName}.jetx.board.freebet`));
    $('.caption-freebet-use').html(GetCaption(`${portalName}.jetx.board.freebet.use`));
    $('.caption-freebet-yes').html(GetCaption(`${portalName}.jetx.board.freebet.yes`));
    $('.caption-freebet-no').html(GetCaption(`${portalName}.jetx.board.freebet.no`));
    $('.caption-freebet-congratulations').html(GetCaption(`${portalName}.jetx.board.freebet.congratulations`));
    $('.caption-freebet-exit').html(GetCaption(`${portalName}.jetx.board.freebet.exit`));
    $('.caption-freebet-cancel').html(GetCaption(`${portalName}.jetx.board.freebet.cancel`));

    $('.caption-freebet-text1').html(GetCaption(`${portalName}.jetx.board.freebet.text1`));
    $('.caption-freebet-text2').html(GetCaption(`${portalName}.jetx.board.freebet.text2`));
    $('.caption-freebet-text3').html(GetCaption(`${portalName}.jetx.board.freebet.text3`));
    $('.caption-freebet-text4').html(GetCaption(`${portalName}.jetx.board.freebet.text4`));
    $('.caption-freebet-text5').html(GetCaption(`${portalName}.jetx.board.freebet.text5`));
    $('.caption-freebet-text10').html(GetCaption(`${portalName}.jetx.board.freebet.text10`));

    $('.caption-spin-amount').html(GetCaption(`${portalName}.jetx.board.gift.spin.amount`)); //
    $('.caption-spin-win').html(GetCaption(`${portalName}.jetx.board.gift.spin.win`));
    $('.caption-spin-count').html(GetCaption(`${portalName}.jetx.board.gift.spin.count`));
    $('.caption-spin-used').html(GetCaption(`${portalName}.jetx.board.gift.spin.used`)); //
    $('.caption-spin-left').html(GetCaption(`${portalName}.jetx.board.gift.spin.left`));

    captionFreeBetText4 = GetCaption(`${portalName}.jetx.board.freebet.text4`);
    captionFreeBetUse = GetCaption(`${portalName}.jetx.board.freebet.use`);
    captionFreeBetText4Continue = GetCaption(`${portalName}.jetx.board.freebet.text4.continue`);
    captionFreeBetContinue = GetCaption(`${portalName}.jetx.board.freebet.continue`);
}

let IsNetworkOptimized = false;
if($('#IsNetworkOptimized') && $('#IsNetworkOptimized').val() === 'True') {
    IsNetworkOptimized = true;
}
let IsJetXNewChatEnabled = false;
if($('#IsJetXNewChatEnabled') && $('#IsJetXNewChatEnabled').val() === 'True') {
    IsJetXNewChatEnabled = true;
}
let IsJetXApiEnabled = false;
if($('#IsJetXApiEnabled') && $('#IsJetXApiEnabled').val() === 'True') {
    IsJetXApiEnabled = true;
}

let InactivityTime = -1;
if($('#InactivityTime')) {
    InactivityTime = parseFloat($('#InactivityTime').val());
}

let InactivityKickTime = -1;
if($('#InactivityKickTime')) {
    InactivityKickTime = parseFloat($('#InactivityKickTime').val());
}
...
let timeDifference = (() => {
    ...
    try {
        ...
        difference = (new Date()).getTime() - (new Date(currentTime)).getTime();
...
function formatNumberWithSpaces(number) {
    let formattedNumber = new Intl.NumberFormat('en-US', {
        minimumFractionDigits: player.currency.fractionDigit,
        maximumFractionDigits: player.currency.fractionDigit
    }).format(number);
...
function InitializeUrls(baseUrl, connrollerName, externalUrl) {
    baseUrl = "https://ldr-w7.ssgmrr31.xyz";
    urlHolder =
        {
            Chat:
                {
...
function checkboxCheck() {
    let checked = false;
    for (let i = 0; i < 2; i++) {
        if ($(`#auto-bet-${i}`).is(':checked')) checked = true;
        if ($(`#auto-cash-out-${i}`).is(':checked')) checked = true;
    }
    if (!checked) {
        $('.cash-game').addClass('simple');
        $(`#auto-bet-all`).prop('checked', false);
        $(`#auto-cash-out-all`).prop('checked', false);
    } else {
        $('.cash-game').removeClass('simple');
    }

    for (let i = 0; i < 2; i++) {
        if ($(`#auto-cash-out-${i}`).is(':checked')) {
            $(`#button-${i} .bet-input.cash-out .input`).removeClass('disabled');
        } else {
            $(`#button-${i} .bet-input.cash-out .input`).addClass('disabled');
        }
    }
}
...
function helpHtml() {
    let caption = `jetx.board.rules.content.new`;
    if (jurisdictionName === 'br') caption = `br.${caption}`;
    else if (jurisdictionName === 'pt') caption = `pt.${caption}`;
    else if (jurisdictionName === 'es') caption = `es.${caption}`;
    let helpHtml = GetCaption(caption);

    let v = GetGameVersion();
    if (v === 'v.2.2.0.0') {
        helpHtml = helpHtml.replace('96.7-98.8', '96.2-98.9');
        helpHtml = helpHtml.replace('96,7-98,8', '96.2-98.9');
    }

    if (jurisdictionName === 'pt') {
        // helpHtml = helpHtml.replaceAll('1000', '100');
        // helpHtml = helpHtml.replaceAll('1,000', '100');
    }
...
function PostCustomEvent(token, eventCode, position = 0) {
    if(IsJetXApiEnabled) {
        ...
        window.hub.invoke('Post', window.token, code).done(function (data) {
            if (data.ErrorMessage) {
                OnRequestFail(data, position);
            } else {
                if (eventCode === 'jetx.gifts.start') {
...
function BetType(amount, autocashout, position) {
    ...
    this.AutoPlay = $(`#auto-bet-${position}`).is(':checked');
}
...
function PlaceBet(token, betAmount, autocashout, position) {
    if (!jetX.game.loader) {
        if (position > 1) {
            let placedCount = (player.prevBets[2] ? 1 : 0) + (player.prevBets[3] ? 1 : 0);
            if (player.gift.count > placedCount) {
                player.prevBets[position] = true;
                playerButtons();
            }
        } else {
            if (player.prevBetSum + parseFloat(betAmount) <= player.availableAmount) {
                player.prevBetSum += position < 2 ? parseFloat(betAmount) : 0;
                player.prevBets[position] = true;
                playerButtons();
            } else {
                buttonMessage(position, GetCaption("jetx.board.amount.error"), 1500, 'error');
            }
        }
        setPrevBets();
        return false;
    ...
    if(IsJetXApiEnabled) {
        ...
        let betType = {
            ...
            Position: position,
            AutoPlay: $(`#auto-bet-${position}`).is(':checked'),
        };
...
function OnRequestFail(data, button) { //jqXHR, textStatus, errorThrown, button
    ...
    if (ExceptionMessage === 'error.jetx.not.enough.money') {
        ...
        var Message = GetCaption(ExceptionMessage);
        var html = '';
        html += Message;
        ...
        buttonMessage(button, GetCaption("operator.unreachable"), 1500, 'error');
    } else if (ExceptionMessage === 'error.invalid.jetx.token') {
        var Message = GetCaption(ExceptionMessage).format(player.displayName);
        var html = '';
        html += Message;
        html += '<br><br><a class="popup-bt" onclick="leaveGame();">' + GetCaption('jetx.board.exit') + '</a>';
        $('.popup-content').html(html);
...
function buttonMessage(button, message, delay, status) {
    if (mobile) {
        if (status === 'cashout') {
            ...
            el.find('.win-value').html(`${message.winAmount}<span>${player.currency.currencyCode}</span>`);
            el.removeAttr('style');
...
let player = {
    ...
    availableAmount: 0,
    bets: [],
    prevBets: [false, false, false, false],
    prevBetSum: 0,
    win: 0,
    totalBet: 0,
    activeBet: 0,
...
function setPrevBets() {
    ...
    localStorage.setItem(`prevBets`, player.prevBets);
    localStorage.setItem(`prevBetSum`, player.prevBetSum);
}

function getPrevBets() {
    if (!localStorageAllow) return false;
    let prevBets = localStorage.getItem(`prevBets`);
    let prevBetSum = localStorage.getItem(`prevBetSum`);
    if (prevBets) {
        prevBets = prevBets.split(',');
        for (let i = 0; i < prevBets.length; i++) {
            player.prevBets[i] = prevBets[i] === 'true';
        }
    }
    if (prevBetSum) {
...
function loadData(data, load = false) {
    if(IsJetXNewChatEnabled && data.ClientChatKey) window.parent.postMessage({name: "chat-load", key: data.ClientChatKey}, "*");
    ...
    $('.canvas canvas').css('pointer-events', 'none');
    ...
    if (load) {
        ...
        let helpButton = `
            <div class="help-bet-auto">${GetCaption("jetx.board.aut")} ${GetCaption("jetx.board.bet")}</div>
            <div class="help-cash-out-auto">${GetCaption("jetx.board.aut")} ${GetCaption("jetx.board.cashout")}</div>
            <div class="help-amount">${amount}</div>
            <div class="help-coef">${coef}</div>
            <div class="help-bet-list-0">${betList[0]}</div>
            <div class="help-bet-list-1">${betList[1]}</div>
            <div class="help-bet-list-2">${betList[2]}</div>
            <div class="help-bet-list-3">${betList[3]}</div>
        `;
        let help1 = `
            ${helpButton}
            <div class="help-place-bet-button">
                <div class="text1">${formatNumberWithSpaces(amount)} <span>${amountCurrency}</span></div>
                <div class="text2">${GetCaption("jetx.board.placebet")}</div>
            </div>
        `;
        let help2 = `
            <div class="help-cash-out-coef" style="top: ${helpCoefTop}px">${board.maxCashoutCoeff}</div>
        `;
        let help3 = `
            ${helpButton}
            <div class="help-cash-out-button">
                <div class="text1">${formatNumberWithSpaces(board.maxWinAmount)} <span>${amountCurrency}</span></div>
        ...
        inputCurrency($(`#cash-out-value-0`));
        inputCurrency($(`#cash-out-value-1`));
    }
}
...
function loadPlayerData(data, load = false) {
    let prevGiftStarted = player.gift.started;
    ...
    for (let buttonIndex = 0; buttonIndex < player.bets.length; buttonIndex++) {
        if ($('#bet-' + buttonIndex).hasClass('disable')) {
            gameEvent.updateBet();
        }
    }
    ...
    if (load) {
        ...
        $('.bet-list a').each(function () {
            if ($(this).data('bet') != '0') {
                if (jurisdictionName === 'pt') {
                    $(this).html(parseInt($(this).data('bet')));
                } else {
    ...
    // console.log(`startAmount = ${player.gift.startAmount}, started=${player.gift.started}, totalWin=${player.gift.totalWin}, prevGiftStarted=${prevGiftStarted}`);
    // if (load) {
    ...
    // if (!player.gift.started && player.gift.totalWin > 0) { // && player.gift.totalWin > 0
    //     showGiftPopup('win');
    // }

    if (prevGiftStarted && !player.gift.started) {
        showGiftPopup('win');
    } else {
        if (player.gift.startAmount > 0 && ((!player.gift.started && player.gift.totalWin === 0) || load)) {
            if (!player.gift.started && player.gift.totalWin === 0) giftLoaded = false;
...
function playerButtons(load = false) {
        ...
        for (let i = 0; i < player.bets.length; i++) {
            ...
            if (load) {
                if (activeButton === 'cancel-bet' || activeButton === 'cashout' || activeButton === 'disable') {
                    ...
                    if (lastBet !== null) {
                        $(`#auto-bet-${i}`).prop('checked', lastBet.autoBet);
                        if (lastBet.amount !== '') $(`#bet-value-${i}`).val(lastBet.amount);
                        $(`#auto-cash-out-${i}`).prop('checked', lastBet.autoCashOut);
                        if (lastBet.cashOut !== '') $(`#cash-out-value-${i}`).val(lastBet.cashOut);
                    }
                }
            }
            ...
            if (activeButton !== 'placement' && activeButton !== 'cancel-bet' && activeButton !== 'cashout' && activeButton !== 'disable') {
                ...
                if (button.hasClass('cancel-bet') && status === 'cash-out') {
                    ...
                    if (status === 'cancel-bet' || status === 'cash-out' || status === 'disable') {
                        ...
                        if (status !== 'cancel-bet') autoCashOutCheckbox.prop('checked', autoCashout);
                        checkboxCheck();
                        if (autoCashout) {
                            if (parseFloat(autoCashOutInput.val()) !== parseFloat(autoCashoutValue)) {
                                autoCashOutInput.val((Math.floor(autoCashoutValue * 100) / 100).toFixed(2));
                                setInputValue(`cash-out-value-${i}`);
                                inputCurrency($(`#cash-out-value-${i}`));
                            }
                        }
                    }
                }
    ...
    $(`#button-all`).removeClass('disabled');
    if (disabled) $(`#button-all`).addClass('disabled');
...
function addCurrentSpin() {
    ...
    if (currentSpinInfo.Info !== '') {
        if (mobile) {
            $('.history-list').prepend(`<div id="${currentSpinInfo.SpinHash}" data-info='${JSON.stringify(currentSpinInfo)}' class="history-item current" data-attr="hash current"><span class="history-logo"></span></div>`);
        } else {
            $('#last100Spins').prepend(`<div id="${currentSpinInfo.SpinHash}" data-info='${JSON.stringify(currentSpinInfo)}' class="row current" data-attr="hash current"><span class="history-logo"></span></div>`);
        }
    }
}
...
function setCurrentSpin(value) {
    ...
    $('.history-list .history-item.current').html(`${coefficient}<span class="coef">x</span>`).css('min-width', `${41 + 6 * (coefficient.length - 4)}px`);
    $('#last100Spins .row.current').html(`${coefficient}<span class="coef">x</span>`);
}
...
function last100Spins() {
    ...
    for (let i = 0; i < history.last100Spins.length; i++) {
        ...
        if (mobile) {
            html += `<div id="h-${spin.SpinHash}" data-info='${JSON.stringify(spin)}' class="item ${status}">${coefficient}<span class="coef">x</span></div>`;

            if (i < 25) {
                headerHtml += `<div id="${spin.SpinHash}" data-info='${JSON.stringify(spin)}' class="history-item ${status}" style="min-width: ${41 + 6 * (coefficient.length - 4)}px" data-attr="mobile hash open">${coefficient}<span class="coef">x</span></div>`;
            }
        } else {
            html += `<div id="${spin.SpinHash}" data-info='${JSON.stringify(spin)}' class="row ${status}" data-attr="hash open">${coefficient}<span class="coef">x</span></div>`;
        }
    }
...
function last100SpinsInfo(spin) {
    ...
    if (parseFloat(coefficient) > 0) {
        coefficient = `${parseFloat(spin.Coefficient).toFixed(2)}<span class="coef">x</span>`;
    }
            ...
            <div class="hash-popup">
                <div class="hash-popup-content">
                    <div class="head">
                        ...
                        <span class="close" data-attr="hash close">
...
function last24ClientSpins() {
    ...
    for (let i = 0; i < history.last24ClientSpins.length; i++) {
        ...
        cashoutCoefficient = cashoutCoefficient === 0 ? '--' : `${cashoutCoefficient.toFixed(2)}<span class="coef">x</span>`;
        wonAmount = formatAmount(wonAmount, {currencyCode, fractionDigit});

        html += `
            <div class="row${status}">
                <div class="col1"><div>${changeTime(spinTimeParsed, 'time')}</div></div>
                <div class="col3">${coefficient}<span class="coef">x</span></div>
...
function last20BigWins() {
    ...
    for (let i = 0; i < history.last20BigWins.length; i++) {
        ...
        <div class="row">
            ...
            <div class="col2">${betAmount}</div>
            <div class="col3">${cashoutCoefficient}<span class="coef">x</span></div>
            <div class="col4">${wonAmount}</div>
        </div>
        `;
    }
    ...
    if (
        ...
        if(type !== 'Coeffs') {
            for(let i = 0; i < list.length; i++) {
                let spin = list[i];
                if(spin.CashoutCoefficient > board.maxCashoutCoeff) {
                    spin.CashoutCoefficient = Math.min(spin.CashoutCoefficient, board.maxCashoutCoeff);
                    spin.WonAmount = spin.BetAmount * spin.CashoutCoefficient;
                    spin.Coefficient = Math.min(spin.Coefficient, board.maxCashoutCoeff);
                }
            }
            if(type === 'HugeWins') {
                list.sort((a, b) => b.WonAmount - a.WonAmount);
            } else if(type === 'BigWins') {
                list.sort((a, b) => b.CashoutCoefficient - a.CashoutCoefficient);
            }
        }
        ...
        for(let i = 0; i < list.length; i++) {
            ...
            if(type === 'Coeffs') {
                let spinDate = spin.SpinDate;
                let coefficient = (Math.min(spin.WonAmount, board.maxCashoutCoeff)).toFixed(2);
                ...
                html += `
                <div class="row">
                    <div class="col1"><div>${changeTime(`${spinDate}`)}</div></div>
                    <div class="col2">${coefficient}<span class="coef">x</span></div>
                </div>
                `;
            } else {
                let currency = spin.Currency;
                ...
                let wonAmount = spin.WonAmount;
                let spinTimeParsed = spin.SpinTimeParsed;
                let cashoutCoefficient = (spin.CashoutCoefficient).toFixed(2);
                let coefficient = spin.Coefficient;
                let balance = spin.Balance;
                let bets = spin.Bets;
                let clientName = formatClientName(spin.ClientName);
...
function showGiftPopup(type) {
    ...
    $('.gift-win-title').css('opacity', 1);

    if (type === 'new') {
        let count = player.gift.count;
        let headCaption = captionFreeBetText4;
        let acceptCaption = captionFreeBetUse;
        if (player.gift.count !== player.gift.startAmount) {
            headCaption = captionFreeBetText4Continue;
            acceptCaption = captionFreeBetContinue;
        ...
        if (parseFloat(player.gift.totalWin) === 0) $('.gift-win-title').css('opacity', 0);
...
function showGiftTab(load = false) {
    ...
    if (!active) {
        $('.buttons .tabs').removeClass('none');
        $('.canvas').addClass('active-tabs');
    }
...
function hideGiftTab() {
    if (!$('.buttons .tabs').hasClass('none')) {
        ...
        $('.canvas').removeClass('active-tabs');
    }
}
...
function giftButtonCancel() {
    $('.gift-popup').removeClass('gift-status-new gift-status-win gift-status-cancel').addClass('gift-status-cancel');
    $('.gift-new').addClass('none');
    $('.gift-cancel').removeClass('none');
}

function giftButtonCancelYes() {
    $('.gift-popup').fadeOut(100);
    PostCustomEvent(token, 'jetx.gifts.cancel');
}

function giftButtonCancelNo() {
...
function PlaySound(index, type, params) {
    ...
    audioElement[index] = createjs.Sound.play(type, {
...
function mobileSound(type) {
    ...
    if ($('#info-on-start').is(':visible') || $('#help-popup').is(':visible')) {
...
function setInputValue(inputId, defaultValue = '') {
    ...
    if (value === '' && defaultValue !== '') {
        ...
        if (jurisdictionName === 'pt' && el.data('maxbet') !== undefined) value = Math.floor(value);
    ...
    if (betButtonId !== ``) {
        $(betButtonId).html(`${formatNumberWithSpaces(value)}<span class="currency">${currencyFormat(player.currency.currencyCode)}</span>`);
    }
    inputCurrency(el);
}
...
function keyboardCursor(inputId) {
    ...
    if (inputId.indexOf('cash-out-value') >= 0) {
        inputWidth -= 3;
    }
...
function keyboard(inputId) {
    ...
    inputEl.parent().parent().addClass('keyboard-on');
...
function footerPosition() {
    ...
    if ($(window).width() >= $(window).height()) {
        ...
        if (height + headerHeight + paddingTop + 5 > gameHeight) {
            height = gameHeight - headerHeight - 10 - paddingTop - 5;
        }
        height = Math.min(gameHeight * (45 / 100), height);

        footerTop = `${height + $('header').outerHeight() + paddingTop + 5}px`;
        ...
        let historyHeight = $('.header-history').height();
        let buttonsHeight = $('.buttons').height();
        let footerHeight = $('footer').height();
        let gameHeight = headerHeight + historyHeight + buttonsHeight + footerHeight;

        if (windowHeight < height + gameHeight) {
            height -= height + gameHeight - windowHeight;
        }
    }
...
$(window).on('load', function (e) {
    for (let position = 0; position < 2; position++) {
        if (jurisdictionName.toLocaleLowerCase() === 'nl') {
    ...
    $(document).on(clickEvent, '.hide-bets input', function () {
        showBets = !$('#hide-bets').prop('checked');
        ...
        PostCustomEvent(token, 'jetx.show.player.bets');
    });
    ...
    $(document).on(clickEvent, '.button.menu-icon', function () {
    ...
    $(document).on(clickEvent, '.button.chat-icon', function () {
        if (mobile) exitFullscreen();
    ...
    $(document).on(clickEvent, '.menu-div', function (event) {
        if (event.target === event.currentTarget) {
            $('.menu-div').css('display', 'none');
        }
    });

    $(document).on(clickEvent, '.menu-div a.how-to-play', function () {
        $('.menu-div').css('display', 'none');
    ...
    $(document).on(clickEvent, '.menu-div a.rules', function () {
        $('.menu-div').css('display', 'none');
        $('#help-popup').fadeIn(100);
        $('body').addClass('overflow');
        window.parent.postMessage('jetXInfoOpen', '*');
        ...
        $('#help-popup .help-head-item').eq(1).trigger(clickEvent);
    });
    ...
    $(document).on(clickEvent, '#help-popup .rules-item .rules-title', function (event) {
        ...
        $(this).attr('data-attr', `rules ${index + 1}`);
    ...
    */
    ...
    $(document).on(clickEvent, '.sound-menu-a.sound-all .sound-checkbox span', function () {
        ...
        window.parent.postMessage({name: "sound-status", status: !checked}, "*");
        window.parent.postMessage({payload: {settingName: "gameEventSound", settingValue: checked}}, "*");
    ...
    $(document).on(clickEvent, '.buttons .bet', function () {
        let button = $(this).data('button');
        let buttonIndex = Number($(this).attr('data-button'))
        if ($(this).hasClass('place-bet')) {
            if ($(this).hasClass('disabled')) return false;
            $(this).attr('data-attr', `place bet ${buttonIndex + 1}`);
            PlaySound(5, 'PlaceBet');
            //if(autoBetTrigger[button]) $('#bet-value-' + button).trigger('focusout');
            $('#bet-value-' + button).trigger('change');
            $('#bet-value-' + button).trigger('focusout');
            $('#bet-value-' + button).attr('disabled', true);
            $('#bet-value-' + button).attr('disabled', false);
            $('#cash-out-value-' + button).trigger('change');
            $('#cash-out-value-' + button).trigger('focusout');
            $('#cash-out-value-' + button).attr('disabled', true);
            $('#cash-out-value-' + button).attr('disabled', false);
            ...
            $(this).attr('data-attr', `cancel ${buttonIndex + 1}`)
            PlaySound(6, 'PlaceBet');
            //PostCustomEvent(token, 'jetx.undo.bet', button);

            let bet = (button === 2 || button === 3 ? 0 : $('#bet-value-' + button).val());
            player.prevBetSum -= parseFloat(bet);
            player.prevBets[button] = false;
            setPrevBets();
            playerButtons();
        } else if ($(this).hasClass('cash-out')) {
            $(this).attr('data-attr', `collect ${buttonIndex + 1}`)
            Cashout(token, button);
        } else if ($(this).hasClass('disable')) {
            $(this).attr('data-attr', `bet accepted ${buttonIndex + 1}`);
        }
    });
    ...
    $(document).on(clickEvent, '.bet-input .auto input:checkbox', function () {
        let button = $(this).data('button');
        let id = $(this).attr('id');
        if (button === 'all') {
            $('.cash-game').removeClass('simple');
    ...
    $(document).on(clickEvent, '#send-chat', function () {
        if ($('#send-chat').hasClass('active') && !$('#send-chat').hasClass('timer')) {
            SendChatMessage();
    ...
    $('#chat-message').keyup(function (e) {
        ...
        if (e.which === 13) {
            e.preventDefault();
            if ($('#send-chat').hasClass('timer')) return false;
            $('#send-chat').removeClass('active');
            SendChatMessage();
        }
    });
    ...
    $(document).on(clickEvent, '.round-history', function () {
    ...
    if (jurisdictionName === 'br') {
        $('.menu-div a.rules').trigger(clickEvent);
        $('#help-popup .rules-item .rules-title:eq(2)').trigger(clickEvent);
    }
});
...
window.parent.postMessage({name: "chat"}, "*");
...
$(function () {
    let defaultText = '';
    let currency = 1;
    let left = `${17 * currency}px`;
    $(this).next("span").css('left', left);
...
function inputCurrency(el) {
    ...
    if (document.dir === 'rtl') {
        ...
        let width = el.width();
        inputWidth = width / 2 + inputWidth / 2 + left + marginLeft + paddingLeft + 2;
...
let counterIndex = 0;
let setOnlineUsersLoaded = false;
let onlineUsersFinal = 0;
worker.onmessage = function (event) {
    ...
    if (player.bets !== undefined && player.bets !== null) {
        for (let i = 0; i < player.bets.length; i++) {
            ...
            let button = $(`#bet-${i}`);
            if(button.hasClass('cancel-bet') || button.hasClass('cash-out') || button.hasClass('disable')) {
                hasBet = true;
            }
        }
    }
    ...
    window.parent.postMessage({key: "inactivity.check.popup", hasBet: hasBet, inactivity: inactivity}, "*");
...
function setOnlineUsers(count) {
    elements.onlineUsers.innerHTML = count;
}
*/


let onlineUsersTimer = [null, null, null, null, null];
let onlineUsers = 0;
let onlineUsersList = [0, 0, 0, 0, 0];
let currentFrame = [0, 0, 0, 0, 0];
function setOnlineUsers(count) {
    let all = true;
    let currentCount = (count + '').padStart(5, '0');
    if(!setOnlineUsersLoaded) {
        for(let i = 0; i < 5; i++) {
            let current = parseInt(currentCount[i]);
            onlineUsersList[i] = current;

            if(onlineUsersTimer[i]) {
                clearInterval(onlineUsersTimer[i]);
                onlineUsersTimer[i] = null;
            }

            let strip = document.querySelectorAll('.digit-strip')[i];
            let currentDigit = onlineUsersList[i];
            let prevDigit = currentDigit === 0 ? 9 : Math.max(currentDigit - 1, 0);
            let nextDigit = currentDigit === 9 ? 0 : Math.min(currentDigit + 1, 9);
            let digitElements = strip.children;
            digitElements[0].textContent = prevDigit;
            digitElements[1].textContent = currentDigit;
            digitElements[2].textContent = nextDigit;

            document.querySelectorAll('.digit-container')[i].style.display = (current === 0 && all ? 'none' : 'block');
            if(current > 0) {
                all = false;
            }
        }
    ...
    if (onlineUsers === count) return false;
    let duration = count + 20 < onlineUsers ? 5000 : (parseInt(workerTime) - parseInt(workerTime) / 4);
    let plus = count > onlineUsers;
    ...
    for(let i = 0; i < 5; i++) {
        ...
        let prev = parseInt(onlineUsersList[i]);
        let current = parseInt(currentCount[i]);

        if(prev !== current) {
            let frames = Math.abs(current - prev);
            if(plus) {
                if(prev > current) {
                    frames = 10 - prev + current;
                }
            } else {
                if(prev < current) {
                    frames = 10 - current + prev;
                }
            }

            let time = Math.ceil(duration / frames);
            let increment = parseInt(frames / (duration / time)) * (plus ? 1 : -1);
            currentFrame[i] = 0;

            setOnlineUsersItem(i, current, frames, increment, plus, time);
            onlineUsersTimer[i] = setInterval(() => {
                setOnlineUsersItem(i, current, frames, increment, plus, time);
            }, time);
        }
...
function setOnlineUsersItem(i, current, frames, increment, plus = true, time = 200) {
    if(currentFrame[i] >= frames) return false;

    let strip = document.querySelectorAll('.digit-strip')[i];
    let currentDigit = parseInt(onlineUsersList[i]);

    if(currentDigit < 0) currentDigit = 10 + currentDigit;
    else if(currentDigit > 9) currentDigit = currentDigit % 10;
    let prevDigit = currentDigit === 0 ? 9 : currentDigit - 1;
    let nextDigit = currentDigit === 9 ? 0 : currentDigit + 1;
    if(prevDigit < 0) prevDigit = 10 + prevDigit;
    if(nextDigit > 9) nextDigit = nextDigit % 10;
    ...
    digitMove(strip, time, plus);

    onlineUsersList[i] += increment;
    if(onlineUsersList[i] < 0) onlineUsersList[i] = 10 + onlineUsersList[i];
    else if(onlineUsersList[i] > 9) onlineUsersList[i] = onlineUsersList[i] % 10;

    currentFrame[i]++;
    if (currentFrame[i] >= frames) {
        clearInterval(onlineUsersTimer[i]);
        onlineUsersTimer[i] = null;
        onlineUsersList[i] = current;
    }

    onlineUsers = parseInt(onlineUsersList.join(''));
}


function digitMove(strip, time, plus) {
    let height = mobile ? 12 : 22;
    setTimeout(() => {
        strip.style.transition = `transform ${(time - 50) / 1000}s ease-out`;
        strip.style.transform = `translateY(${plus ? -height : height}px)`;
    }, 50)
}
    ...
    <div class="counter-container">
        <div class="digit-container">
            <div class="digit-strip">
                <div class="digit prev-digit">9</div>
                <div class="digit current-digit">0</div>
                <div class="digit next-digit">1</div>
            </div>
        </div>
        <div class="digit-container">
            <div class="digit-strip">
...
window.hub.client.gBoard = (gameInfo) => {
    ...
    for (let i = (!player.disableCashGame ? 0 : 2); i < player.prevBets.length; i++) {
        if (player.prevBets[i]) {
            let bet = (i === 2 || i === 3 ? board.minBet : $(`#bet-value-${i}`).val());
            let cashOut = null;
            if (i < 2) {
                if ($(`#auto-cash-out-${i}`).is(':checked') && $(`#cash-out-value-${i}`).val() !== '') {
                    cashOut = $(`#cash-out-value-${i}`).val();
                }
            }
...
window.hub.client.response = (info) => {
    ...
    let isFinnished = info.f; //IsFinnished
    let value = info.v; //Value
    graphicValue = value; //info.GraphicValue;
    let step = info.s; //Step
    let degree = 1; //info.Degree
    graphicStep = step;
    ...
    if (isFinnished) {
        ...
        let spinHash = info.h ?? board.currentSpinHash;
        jetX.fly();
        currentSpinInfo = {
            SpinTime: GetCaption('jetx.board.hash.time'),
            Coefficient: GetCaption('jetx.board.hash.value'),
...
window.hub.client.g = (updateInfo) => {
    ...
    if(IsNetworkOptimized) {
        let spinInfo = getSpinInfo(updateInfo.I);
        counterId = spinInfo.i;
    }
    if(updateInfo.M === 'c') { //command cashout
        playerCashOut(updateInfo.I);
    } else if (updateInfo.M === 'u') { //undo
        playerUndo(updateInfo.I);
        ...
        playerBet(updateInfo.I); //info
    }
    if(counterId === player.counterId) PlayerInfo(token);
    playersDraw('current');
    playersDraw('');
};
// Hub END
...
let chatVersion = 0;
let reloadVersion = 0;
if(IsJetXNewChatEnabled) {
    window.addEventListener("message", function (event) {
        let message = event.data;
        if (message.name !== undefined) {
            if (message.name === 'chat-receive-message') {
                let messages = [];
                messages.push(message.data)
                AddChatMessages(messages);
            } else if (message.name === 'chat-receive-messages') {
                AddChatMessages(message.data, true);
            }
        }
    })
} else {
...
function LoadChatMessages() {
    ...
    $.get(chatMessageUrl, function (data) {
        ...
        OnMessageRequestFail(jqXHR, textStatus, errorThrown, null);
    });
}
    ...
    if(IsJetXNewChatEnabled) {
        ...
        if (messages !== null) {
            for (let index = 0; index < messages.length; index++) {
                ...
                let chatMessage =
                    ...
                    + ' <div class="chat-date">' + changeTime(message.sendTime, 'timeFull') + '</div>'
                    + ' <div class="chat-user">' + formatClientName(message.senderName) + ':</div>'
                    + ' <div class="chat-text">' + message.message + '</div>'
                    + '</div>';
                let scroll = $('.chat-message').scrollTop() + $('.chat-message').height() + 10 > $('.chat-message')[0].scrollHeight;
        ...
        if (messages !== null) {
            for (let index = 0; index < messages.length; index++) {
                ...
                let chatMessage =
                    '<div class="chat-outer">'
                    + ' <div class="chat-date">' + changeTime(message.SendTimeParsed, 'timeFull') + '</div>'
                    + ' <div class="chat-user">' + formatClientName(message.SenderName) + ':</div>'
                    + ' <div class="chat-text">' + message.Message + '</div>'
                    + '</div>';
                let scroll = $('.chat-message').scrollTop() + $('.chat-message').height() + 10 > $('.chat-message')[0].scrollHeight;
...
function SendChatMessage() {
    if(IsJetXNewChatEnabled) {
        ...
        if (message !== undefined && message !== null && message !== '') {
            window.parent.postMessage({name: "chat-send", message: message}, "*");
        }
        $('#chat-message').val('');
        chatTimeSet();
    } else {
...
function chatTimeSet() {
    if (!localStorageAllow) return false;
    localStorage.setItem(`chat-time-${player.displayName}`, (new Date()).getTime());
}

function chatTimeGet() {
    if (!localStorageAllow) 60;
    let chatTime = localStorage.getItem(`chat-time-${player.displayName}`);
    if (chatTime === null) 60;
    return parseInt(((new Date()).getTime() - parseInt(chatTime)) / 1000);
}

function chatTimerTimer() {
    let lastTime = chatTimeGet();

    $('#send-chat').val(GetCaption('jetx.board.send'));
    $('#send-chat').removeClass('timer');
    if (lastTime < 60) {
        lastTime = 60 - lastTime;
        lastTime = lastTime < 0 ? 0 : lastTime;
        let time = `00:${lastTime < 10 ? `0` : ``}${lastTime}`;
        if (lastTime === 60) time = `01:00`;
        $('#send-chat').addClass('timer').val(time);
    }
}
...
if (skinName !== '') {
    let skins = [];
    skins['IMJBet'] = {color: '#c8de17', width: 105};
    skins['Perabet'] = {color: '#28bc62', width: 105};
    skins['Cbet'] = {color: '#00be7c', width: 105};
    skins['Favbet'] = {color: '#fd267e', width: 110};
    skins['Marsbet'] = {color: '#c52426', width: 117};
    skins['HarifJet'] = {color: '#db7e3a', width: 105};
    skins['RedX'] = {color: '#c3072a', width: 106};
    skins['VenusJet'] = {color: '#f8b218', width: 211};
    skins['PiaJet'] = {color: '#2667c3', width: 155};
    skins['ElexJet'] = {color: '#fffc00', width: 202};
    skins['TulipJet'] = {color: '#f6d98b', width: 160};
    skins['JetXCup'] = {color: '#ffa800', width: 113};
    skins['FogueteX'] = {color: '#d7fc56', width: 140};
    skins['VevoJet'] = {color: '#fbca02', width: 158};
    skins['KTOJet'] = {color: '#fad749', width: 105};
    skins['MeskJet'] = {color: '#723b96', width: 215};
    skins['JojoX'] = {color: '#f4c300', width: 91};
    skins['KlasJet'] = {color: '#28894e', width: 175};
    skins['JetMilbet'] = {color: '#bf36bf', width: 164};
    skins['SafirJet'] = {color: '#6699ff', width: 177};
    skins['JetVera'] = {color: '#1985a1', width: 230};
    skins['BetperJet'] = {color: '#f3ec19', width: 220};
    skins['JetHeybet'] = {color: '#ffcb00', width: 193};
    skins['SavoyJet'] = {color: '#d5a564', width: 172};
    skins['MostX'] = {color: '#fb4d00', width: 144};
    skins['HitbetJet'] = {color: '#f1d50a', width: 143};
    skins['FenomenJet'] = {color: '#ffc700', width: 204};
    skins['BetParkJet'] = {color: '#10815f', width: 101};
    skins['GarantiX'] = {color: '#62af28', width: 119};
    skins['GoraJet'] = {color: '#2ecc71', width: 72};
    skins['JetUp'] = {color: '#ff2400', width: 88};
    skins['JetRest'] = {color: '#b72221', width: 160};
    skins['LidyaJet'] = {color: '#3da2ef', width: 141};
    skins['JetBetpas'] = {color: '#5eb220', width: 214};
    skins['MatJet'] = {color: '#fddf56', width: 185};
    skins['JasminX'] = {color: '#ffa20e', width: 139};
    skins['InterJetX'] = {color: '#d81f27', width: 159};
    skins['Holiganbet'] = {color: '#fdc440', width: 81};
    skins['Mybookie'] = {color: '#ec8825', width: 76};
    skins['Tipobet'] = {color: '#129d48', width: 182};
    skins['WildX'] = {color: '#10cc66', width: 101};
    skins['Vbet'] = {color: '#d80d83', width: 84};
    skins['Parimatch'] = {color: '#f8ff13', width: 103};
    skins['1Win'] = {color: '#2d81ff', width: 105};
    skins['MrBahisJet'] = {color: '#07be5e', width: 203};
    skins['HipodromJet'] = {color: '#e1821b', width: 104};
    skins['OnwinX'] = {color: '#bb10a3', width: 146};
    skins['SahaX'] = {color: '#008000', width: 89};
    skins['BetX'] = {color: '#3fb26e', width: 124};
    skins['JetBeluga'] = {color: '#137272', width: 176};
    skins['JetLiman'] = {color: '#cd9546', width: 144};
    skins['JetHayal'] = {color: '#d99226', width: 176};
    skins['Grandpashabet'] = {color: '#fdff79', width: 193};
    skins['Betwinner'] = {color: '#ffce06', width: 105};
    skins['OleyX'] = {color: '#b60b14', width: 110};
    skins['BestbetJetX'] = {color: '#c22b3c', width: 104};
    skins['KolayX'] = {color: '#f2be1a', width: 136};
    skins['4RABet'] = {color: '#3486e3', width: 113};
    skins['Rajabets'] = {color: '#ffb900', width: 105};
    skins['Bet7k'] = {color: '#adc90e', width: 146};
    skins['JetPix'] = {color: '#f7ef22', width: 109};
    skins['VGBet'] = {color: '#0b9ca7', width: 102};
    skins['Doradobet'] = {color: '#d59c00', width: 103};
    skins['Ecuabet'] = {color: '#ffde00', width: 103};
    skins['TexyBet'] = {color: '#f46b00', width: 103};
    skins['Brxbet'] = {color: '#22b573', width: 103};
    skins['CassinoPix'] = {color: '#0075cd', width: 103};
    skins['Bullsbet'] = {color: '#6acb47', width: 103};
    skins['TurkeyX'] = {color: '#fbde3a', width: 145};
    skins['Shabiki'] = {color: '#f5821f', width: 120};
    skins['WinbirJet'] = {color: '#e62a4d', width: 206};
    skins['Bethub'] = {color: '#ebb058', width: 105};
    skins['Paripesa'] = {color: '#ffaf00', width: 105};
    skins['NGSBahis'] = {color: '#f6ca21', width: 152};
    skins['Konjobet'] = {color: '#c8102e', width: 105};
    skins['Betebet'] = {color: '#f4c000', width: 105};
    skins['Betvole'] = {color: '#3398db', width: 105};
    skins['Br4bet'] = {color: '#70a326', width: 105};
    skins['KingRoyal'] = {color: '#ecb50d', width: 114};
    skins['Galabet'] = {color: '#90cc4c', width: 105};
    skins['Betpipo'] = {color: '#37a54e', width: 105};
    skins['Padisahbet'] = {color: '#24a8e6', width: 105};
    skins['TruvaBet'] = {color: '#c8de17', width: 105};
    skins['Golegol'] = {color: '#0066ca', width: 105};
    skins['Gizabet'] = {color: '#cf0d15', width: 105, helpCoefTop: 38};
    skins['Betoffice'] = {color: '#ff5722', width: 105, helpCoefTop: 38};
    skins['Teslabahis'] = {color: '#08f0b1', width: 105, helpCoefTop: 38};
    skins['Bovada'] = {color: '#c7332d', width: 105, helpCoefTop: 38};
    skins['Dale'] = {color: '#fcc80a', width: 105, helpCoefTop: 38};
    skins['JetBahis'] = {color: '#1fff34', width: 212, helpCoefTop: 38};
    skins['Sesbet'] = {color: '#b28e40', width: 105, helpCoefTop: 38};
    skins['Future9'] = {color: '#e9602a', width: 105, helpCoefTop: 38};
    skins['Favoribahis'] = {color: '#d23627', width: 188, helpCoefTop: 38};
    skins['Makrobet'] = {color: '#fbcd52', width: 170, helpCoefTop: 38};
    skins['Bibubet'] = {color: '#fec115', width: 209, helpCoefTop: 38};
    skins['Milanobet'] = {color: '#92278f', width: 190, helpCoefTop: 38};
    skins['Goldenbahis'] = {color: '#f0bb1d', width: 105, helpCoefTop: 38};
    skins['Bahisbudur'] = {color: '#ee9720', width: 105, helpCoefTop: 38};
    skins['Betcyp'] = {color: '#f4c80f', width: 105, helpCoefTop: 38};
    skins['Tokyobet'] = {color: '#e7363a', width: 105, helpCoefTop: 38};
    skins['Nerobet'] = {color: '#f1e514', width: 105, helpCoefTop: 38};
    skins['Pulibet'] = {color: '#4787d1', width: 105, helpCoefTop: 38};
    skins['Darkbet'] = {color: '#ffc000', width: 105, helpCoefTop: 38};
    skins['KuzeyCasino'] = {color: '#0283fb', width: 105, helpCoefTop: 38};

    let isSkin = false;
    for (let skin in skins) {
        if (skin.toLocaleLowerCase() === skinName.toLocaleLowerCase()) {
            isSkin = true;
            skinName = skin;
            skinColor = skins[skin].color;
            logoWidth = skins[skin].width;
            helpCoefTop = skins[skin].helpCoefTop;
            break;
        }
    }
    ...
    if (isSkin) {
        ...
        document.documentElement.style.setProperty('--logo', `url(${staticContentUrl}../ImagesLight/${skinUrl}${mobile ? '/Mobile' : ''}/logo.png)`);
        document.documentElement.style.setProperty('--help2', `url(${staticContentUrl}../ImagesLight/${skinUrl}/help2.png)`);
        document.documentElement.style.setProperty('--help-down', `url(${staticContentUrl}../ImagesLight/${skinUrl}/help-down.png)`);
        document.documentElement.style.setProperty('--help-up', `url(${staticContentUrl}../ImagesLight/${skinUrl}/help-up.png)`);
...
gameText = {
    graphicValue: {
        ...
        x: 705 - 42,
        y: 303,
        style: {fontFamily: 'robotobold', fontSize: 120, fill: ['#5ee32b'], align: 'center'}
    },
    loading: {
        label: null,
        text: GetCaption('jetx.board.progress').toUpperCase(),
        zOrder: 10,
        x: 705,
        y: 390,
        style: {
...
if (mobile) {
    ...
    gameText.graphicValue.x = 352 - 24;
...
function JetXClass() {
    ...
    this.showInternetLost = function () {
        ...
        if (!gameContainer.ConnectionLost.container.visible) {
            ...
            if (this.cancelBetTimer) {
                clearTimeout(this.cancelBetTimer);
                this.cancelBetTimer = null;
            }
            this.cancelBetTimer = setTimeout(() => {
                $('.cancel-bet').trigger(clickEvent);
            }, 6000);
        }
    };
    ...
    this.drawLoader = function () {
        ...
        gameText.loading.label.text = GetCaption('jetx.board.progress').toUpperCase();
    ...
    this.setMaxMultiplier = function () {
        ...
        let width = gameText.maxMultiplier.label.width + (this.maxMultiplier.maxMultiplierX ? this.maxMultiplier.maxMultiplierX.width : 0);

        gameText.maxMultiplier.label.x = x + width / 2 - 10;
        if (this.maxMultiplier.maxMultiplierX) this.maxMultiplier.maxMultiplierX.x = x + width / 2;
    }
    ...
    this.planeMove = function (value, graphicValue, step, degree) {
        ...
        this.coefficientSetValue(value);
        ...
        if (player.bets !== undefined && player.bets !== null) {
            for (let i = 0; i < player.bets.length; i++) {
                ...
                let amount = formatNumberWithSpaces(formatBalance(value * betAmount));
                if(i > 1 && player.gift.debt > 0) {
                    amount = formatNumberWithSpaces(formatBalance((value - 1) * player.gift.debt)); //player.gift.debt
                }
                button.find('.cash-out1').html(`${amount}<span class="currency">${currencyFormat(player.currency.currencyCode)}</span>`);
            }
        }
        //
    };

    this.coefficientSetValue = function(value) {
        if(!gameInfo.resourceLoaded) return false;
        gameText.graphicValue.label.text = value.toFixed(2);
        let width = gameText.graphicValue.label.width / 2;
        this.graphicX.x = gameText.graphicValue.x + width + 10;
    };
...
(function () {
    ...
    function onchange(evt) {
        ...
        if (jetX.tabActive) {
            ...
            volume = true;
            setOnlineUsersLoaded = false;
            setOnlineUsers(onlineUsersFinal);
        } else {
        ...
        loader.load(function (e, res) {
            ...
            setTimeout(function () {
                ...
                window.parent.postMessage({
                    ...
                    soundPopup: $('.sound-popup').is(':visible')
                }, "*");
    ...
    this.scale = function () {
        ...
        if (mobile) {
            if (!vertical) {
                ...
                this.responsiveY = (-marginTop / (2 * scale)) * 1.3;
            } else {
                // this.responsiveY = 0;
...
window.addEventListener("message", function () {
    ...
    if (message.name !== undefined) {
        if (message.name === 'prize-drop-jackpot-start') {
            ...
            checkboxCheck();
        } else if (message.name === 'update-game-balance') {
            PostCustomEvent(token, 'update.balance');
        }
    }
});
...
function getRowHeight() {
    return mobile ? (vertical ? 27 : 23) : 32;
}
...
function playersDraw(type) {
    if (type === 'current') {
        if (showBets) window.jetXReact.setCurrent(playersCurrent);
    } else {
        ...
        if (showBets) window.jetXReact.setActive(list, activeHeight, activeStart * rowHeight);
        ...
        if (showBets) window.jetXReact.setCashOut(list, cashOutHeight, cashOutStart * rowHeight);
    }
}
...
function getSpinInfo(spin) {
    let counterId, betPlace, current, displayName, betAmount, cashout, winAmount, portalName, currency;
...
function playersInfoItem(spin, small = false, cashouted = false) {
    ...
    if(IsNetworkOptimized) {
        if(spin.a) {
            spin = getSpinInfo(spin);
        }
        if(spin.p === '') board.isPortalFilterOn = false;
    }

    //if(small) {
    currency = spin.r; //currency
    // currencyCode = currency.currencyCode;
    // fractionDigit = currency.fractionDigit;
    currencyCode = currency.split('_')[0];
    fractionDigit = currency.split('_')[1];

    counterId = spin.i; //counterId
    betPlace = spin.x; //betPlace
    current = counterId === player.counterId;
    displayName = formatClientName(spin.d, current); //displayName
    betAmount = spin.b; //betAmount
    cashout = spin.c; //cashout
    winAmount = spin.w; //winAmount
    portalName = spin.p; //portalName
    //} else {
    ...
    if (board.convertToPlayerCurrency) {
        ...
        if (currencyCode !== player.currency.currencyCode) {
            ...
            //         pr = 1;
            //     }
            //     betAmount = Math.floor(betAmount / pr) * pr === 0 ? defBetAmount : Math.floor(betAmount / pr) * pr;
            // }
    ...
    let winAmountFormated = winAmount === 0 ? '--' : numberFormat(winAmount, {currencyCode, fractionDigit});
    ...
    this.receiveMessage = function () {
        ...
        function EventHandler(eventData) {
            if (eventData.data !== undefined && eventData.data.name === "sound-toggle") {
                $('.sound-menu-a.sound-all .sound-checkbox span').trigger('click');
            }
...
$(document).on(clickEvent, function (event) {
    try {
        ...
        // Customize the message data as needed
        let tagName;
        if (target.getAttribute('data-attr') === null) {
            tagName = target.classList.value === '' ? target.tagName.toLowerCase() : target.classList.value
        } else tagName = target.getAttribute('data-attr')
        console.log(tagName);
        var messageData = {
            event: 'iframeClick',
...
if(portalName === 'betlive') {
    if(mobile) mobileSound(true);
} else if(portalName.indexOf('adjarabet') >= 0) {
    if(mobile) mobileSound(true);
}